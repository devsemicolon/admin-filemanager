<div id="filemanager">
  <div class="card">
    <div class="card-header">
      <h4 class="card-title">{{ $data['heading_title'] }}</h4>
    </div>
    <div class="card-body">
      <div class="row">
        <div class="col-sm-5"><a href="{{ $data['parent'] }}" data-toggle="tooltip" title="Parent" id="button-parent" class="btn btn-default"><i class="fa fa-level-up"></i></a> <a href="{{ $data['refresh'] }}" data-toggle="tooltip" title="Refresh" id="button-refresh" class="btn btn-default"><i class="fa fa-refresh"></i></a>
          ...
          <button type="button" data-toggle="tooltip" data-trigger="focus" title="Folder" id="button-folder" class="btn btn-default"><i class="fa fa-folder"></i></button>
...
$('#button-search').on('click', function(e) {
	var url = '{{ url('admin-filemanager?directory='.$data['directory']) }}'
...
$('#button-upload').on('click', function() {
	...
	timer = setInterval(function() {
		if ($('#form-upload input[name=\'file[]\']').val() != '') {
			...
			$.ajax({
				url: '{{ url('admin-filemanager/upload?directory='.$data['directory']) }}',
				type: 'post',
				dataType: 'json',
...
$('#button-folder').popover({
	...
	content: function() {
		...
		html += '  <input type="text" name="folder" placeholder="Folder Name" class="form-control">';
...
$('#button-folder').on('shown.bs.popover', function() {
	$('#button-create').on('click', function() {
		$.ajax({
			url: '{{ url('admin-filemanager/folder?directory='.$data['directory']) }}',
			type: 'post',
			dataType: 'json',
...
$('#container-fluid-data #button-delete').on('click', function(e) {
...
$( document ).ready(function() {
	...
	function loadMoreData(page){
		...
		$.ajax({
			url: "{{ url('admin-filemanager/pagination') }}?page="+ page + target + directory + filter,
			type: "get",
