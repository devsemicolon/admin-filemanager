...
class AdminFileManagerController extends Controller
{
    ...
    public function index(Request $request) {
        ...
        $data['parent'] = url('admin-filemanager?1=1'. $url);
        ...
        $data['refresh'] = url('admin-filemanager?1=1'. $url);
        ...
        return view('admin-filemanager::admin-filemanager',compact('data'));
    }
    ...
    public function pagination(Request $request) {
        ...
        foreach ($images as $image) {
            ...
            if (is_dir($image)) {
                ...
                $data['images'][] = array(
                    ...
                    'href'  => url('admin-filemanager?directory=' . urlencode(mb_substr($image, mb_strlen($this->storage_path.'/'))) . $url)
                );
            } elseif (is_file($image)) {

                $image_name = implode(' ', $name);
                $data['images'][] = array(
                    ...
                    'name'  => strlen($image_name) > 25 ? substr($image_name,0,25)."..." : $image_name,
                    'type'  => 'image',
                    'path'  => mb_substr($image, mb_strlen($this->storage_path)),
        ...
        $data['heading_title'] = 'File Manager';
        return view('admin-filemanager::data',compact('data'));
    }
